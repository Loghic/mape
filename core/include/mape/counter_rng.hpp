...
inline std::uint64_t counter_bits(std::uint64_t key, std::uint64_t counter) {
    std::uint64_t z =
        key * 0x9E3779B97F4A7C15ULL + counter + 0xD1B54A32D192ED03ULL;
...
inline double counter_uniform(std::uint64_t key, std::uint64_t counter) {
    ...
    const double u =
        (static_cast<double>(bits) + 0.5) * (1.0 / 9007199254740992.0);
    return u;  // in (0, 1)
}

// Inverse standard-normal CDF (Acklam's rational approximation, |err|
// < 1.2e-9). Maps a uniform draw to a standard normal — the counter-based
// analogue of std::normal_distribution, but stateless and reproducible per
// index.
